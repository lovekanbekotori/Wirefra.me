...
void CVisuals::Draw()
{
	for (int i = 1; i < 65; i++)
	{
		...
		CColor health_color = CColor(131, 175, 249, alpha[entity->GetIndex()]);
		CColor dormant_color = CColor(100, 100, 100, alpha[entity->GetIndex()]);
		CColor box_color;
		CColor spread_color = CColor(30, 30, 30, 45);
		CColor still_health;
		CColor alt_color;
		CColor zoom_color;
		CColor bomb_color;
		...
		else if (!entity->GetIsDormant())
		{
			...
			main_color = SETTINGS::settings.health_col; //heath_color
...
void CVisuals::DrawFovArrows(SDK::CBaseEntity* entity)
{
	...
	mopvar::Engine->GetViewAngles(client_viewangles);
	...
	vertices.push_back(SDK::Vertex_t(Vector2D(screen_center.x + cosf(rot + DEG2RAD(2)) * (radius - 24), screen_center.y + sinf(rot + DEG2RAD(2)) * (radius - 16))));
	vertices.push_back(SDK::Vertex_t(Vector2D(screen_center.x + cosf(rot - DEG2RAD(2)) * (radius - 24), screen_center.y + sinf(rot - DEG2RAD(2)) * (radius - 16))));

	RENDER::TexturedPolygon(3, vertices, CColor(131, 175, 248)); //255, 40, 230
}
...
void CVisuals::DrawSniperCrosshair()
{

	static char* DrawCrosshairWeaponTypeCheck = reinterpret_cast<char*>(UTILS::FindSignature("client.dll", "83 F8 05 75 17"));
...
void CVisuals::DrawBulletBeams()
{
	...
	for (int i = 0; i < Impacts.size(); i++)
	{
		...
		beamInfo.m_flGreen = current.color.RGBA[0];
...
void CVisuals::DrawIndicator()
{
	...
	if (SETTINGS::settings.antiaim_type == 2 || SETTINGS::settings.antiaim_type == 4)
	{
		...
		{
			//ENDER::DrawF((screen_width / 2) - 40, screen_height / 2, FONTS::visuals_side_font, true, true, CColor(10, 145, 190, 255), "<"); //green
			//RENDER::DrawF((screen_width / 2) + 40, screen_height / 2, FONTS::visuals_side_font, true, true, CColor(255, 255, 255, 255), ">");
		}
	}
}
