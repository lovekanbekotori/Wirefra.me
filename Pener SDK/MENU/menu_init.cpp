...
namespace MENU
{
	void InitColors()
	{
		...
		colors[GAME_SNEEZE] = CColor(131, 175, 248);

		colors[WINDOW_BODY] = CColor(30, 30, 30);
		...
		colors[CHECKBOX_CLICKED] = CColor(131, 175, 248, 255);
		...
		colors[SLIDER_VALUE] = CColor(131, 175, 248, 255); //HOTPINK
	...
	void Do()
	{
		...
		else
		{
			if (menu_open)
			{
				...
				WindowBegin("Wireframe", Vector2D(200, 200), Vector2D(420, 375));
				...
				std::vector<std::string> aa_mode = { "none", "Backwards", "Sideways", "bigdaddyAA", "Lowerbody", "pysenAA", "xplayAA" };
				...
				case 0:
				{
					Checkbox("Activate", SETTINGS::settings.aim_bool);
					{
						GroupboxBegin("Main", 227.5, 1);
						{
							...
							Checkbox("Serversided Silent Aim", SETTINGS::settings.reverse_bool);
				...
				case 1:
				{
					Checkbox("Enable", SETTINGS::settings.esp_bool);
					if (SETTINGS::settings.esp_bool)
					{
						...
						if (SETTINGS::settings.fov_bool)
							ColorPicker("arrow color", SETTINGS::settings.fov_col);
												Checkbox("Sniper Crosshair", SETTINGS::settings.snipercross_bool);
				...
				case 3:
				{
				/*	GroupboxBegin("Misc", 95, 1);
					Checkbox("Activate", SETTINGS::settings.aa_bool);
					{
						Checkbox("Wall Detection", SETTINGS::settings.walldetection);

						Combobox("Anti-aim type", aa_mode, SETTINGS::settings.aa_type);
						if (SETTINGS::settings.aa_type == 4 || SETTINGS::settings.aa_type == 5)
