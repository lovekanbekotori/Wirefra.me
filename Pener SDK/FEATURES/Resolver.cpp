...
void CResolver::resolve(SDK::CBaseEntity* entity)
{
	...
	{
		using_fake_angles[entity->GetIndex()] = entity->GetSimTime() - old_simtime[entity->GetIndex()] == mopvar::Globals->interval_per_tick;
...
void CResolver::resolve(SDK::CBaseEntity* entity)
{
	...
	auto server_time = local_player->GetTickBase() * mopvar::Globals->interval_per_tick; //i have a global dedicated to curtime but am using this because lemon is gay
