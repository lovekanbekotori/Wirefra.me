...
void CGameEvents::PlayerHurtListener::FireGameEvent(SDK::IGameEvent* game_event)
{
	...
	{
		hitmarker_time = 255;
		if(SETTINGS::settings.hitsound == 1)
		PlaySoundA(reinterpret_cast<const char*>(hitsound_byte_array), nullptr, SND_MEMORY | SND_ASYNC);
		else if(SETTINGS::settings.hitsound == 2)
		mopvar::Surface->IPlaySound("buttons\\arena_switch_press_02.wav");
		else if (SETTINGS::settings.hitsound == 3)
		PlaySoundA(reinterpret_cast<const char*>(bubble), nullptr, SND_MEMORY | SND_ASYNC);
