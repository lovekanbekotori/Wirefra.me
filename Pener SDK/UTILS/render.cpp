...
namespace RENDER
{
	...
	void DrawFilledQuadOneSided(Vector2D p0, Vector2D p1, Vector2D p2, Vector2D p3, CColor c)
	{
		std::vector<SDK::Vertex_t> vertices;

		static int Texture = mopvar::Surface->CreateNewTextureID(true);
		byte buffer[4] = { 255, 255, 255, 255 };
		mopvar::Surface->DrawSetTextureRGBA(Texture, buffer, 1, 1);
		mopvar::Surface->DrawSetColor(c);
		mopvar::Surface->DrawSetTexture(Texture);

		vertices[0].Init(p0);
		vertices[1].Init(p1);
		vertices[2].Init(p2);
		vertices[3].Init(p3);
		mopvar::Surface->DrawTexturedPolygon(4, vertices.data());
	}
